...
**核心整合思路：**
我们将 `doc/plan.md` 中的 **Orchestrator (编排器)** 确立为系统的核心调度中枢，而将 `doc/合并.md` 中的 **"Turn-based Tree"** 及其数据访问层封装为 **History Engine (历史引擎)**，作为 Data Layer 的关键子系统。

这种融合解决了原计划中关于“历史如何管理”的模糊地带：
1.  **Orchestrator** 不再直接操作字符串列表，而是持有 **Session Pointer (指针)**。
...
sequenceDiagram
    ...
    Note over U, O: 阶段 1: 输入与锚定
    U->>O: 发送消息 "攻击哥布林"
    O->>H: createNode(parent=Current, content="攻击")
    H-->>O: 返回 NewNodeID
    O->>O: 更新指针 -> NewNodeID

    Note over O, P: 阶段 2: 规划与装配
    O->>H: getLinearHistory(NewNodeID)
    H-->>O: 返回 [Msg1, Msg2, ..., "攻击"]
    O->>P: 请求决策 (Need Lore? Need State?)
    P-->>O: 决策信号
    O->>O: 构建 PromptAST (注入 Lore/State)

    Note over O, G: 阶段 3: 生成
    O->>G: 发送最终 Prompt
    G-->>O: 流式返回 "你挥剑击中了..."
    O->>U: 实时更新 UI
    O->>H: createRecord(node=NewNodeID, content=...)

    Note over O, PP: 阶段 4: 结算与持久化
    O->>PP: 分析回复内容
    PP-->>O: 提取状态变更 {HP: -10}
...
erDiagram
    ...
    ContextNode {
        ...
        UUID parent_id FK
        TEXT user_content
        JSON state_snapshot "关键：此时刻的世界状态"
        UUID selected_record_id "当前选中的回复"
        DATETIME created_at
    }
